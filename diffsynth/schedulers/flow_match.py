...
class FlowMatchScheduler():
    ...
    def add_noise(self, original_samples, noise, timestep):
        if not isinstance(timestep, torch.Tensor):
            timestep = torch.tensor(timestep, device=original_samples.device, dtype=torch.float)
        
        self.timesteps = self.timesteps.to(timestep.device)
        self.sigmas = self.sigmas.to(timestep.device)

        dists = (timestep.unsqueeze(1) - self.timesteps.unsqueeze(0)).abs()
        timestep_ids = torch.argmin(dists, dim=1)
